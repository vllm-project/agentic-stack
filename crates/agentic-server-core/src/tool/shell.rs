...
impl ShellHandler {
    ...
    pub(crate) fn output_item(call: &FunctionToolCall) -> Option<OutputItem> {
        Self::output_item_with_status(call, ShellCallStatus::InProgress)
    }

    #[must_use]
    pub(crate) fn output_item_with_status(call: &FunctionToolCall, status: ShellCallStatus) -> Option<OutputItem> {
        let action = serde_json::from_str::<ShellCallAction>(&call.arguments).ok()?;
...
pub(crate) fn insert_shell_entry(entries: &mut HashMap<String, ToolEntry>, _params: &ShellToolParam) {
...
mod tests {
    ...
    fn normalized_function_call_restores_shell_call() {
        ...
        assert_eq!(shell.status, Some(ShellCallStatus::InProgress));
    }
