...
try:
    ...
    raise RuntimeError(
        "OpenTelemetry logging support requires optional dependencies. Install via `agentic_stack[tracing]`."
    ) from e
...
# Severity mapping
SEVERITY_MAPPING = {
    5: (SeverityNumber.TRACE, "TRACE"),
    10: (SeverityNumber.DEBUG, "DEBUG"),
    20: (SeverityNumber.INFO, "INFO"),
    25: (SeverityNumber.INFO4, "SUCCESS"),
...
class OTLPHandler:
    ...
    def _create_log_record(self, record: Dict[str, Any]) -> LogRecord:
        ...
        severity_number, severity_text = SEVERITY_MAPPING.get(
            record["level"].no, (SeverityNumber.UNSPECIFIED, "UNSPECIFIED")
        )
